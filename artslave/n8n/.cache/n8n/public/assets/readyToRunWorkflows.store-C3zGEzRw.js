...
`,options:{}},type:`n8n-nodes-base.gmail`,typeVersion:2.1,position:[1264,-192],id:`4a23b2b5-21f7-4a63-a1ab-9f18d6ce2e6a`,name:`Send output via email using Gmail`,webhookId:`65c7b462-bb4a-400c-a556-ef408efcd208`,notesInFlow:!0,notes:`Double-click here to connect!`}],pinData:{},connections:{"Get city latitude and longitude":{main:[[{node:`Limit`,type:`main`,index:0}]]},"Get weather":{main:[[{node:`Get today's high and low`,type:`main`,index:0}]]},"Set your location":{main:[[{node:`Get city latitude and longitude`,type:`main`,index:0}]]},Merge:{main:[[{node:`Aggregate`,type:`main`,index:0}]]},Aggregate:{main:[[{node:`Select output for the email`,type:`main`,index:0}]]},"Get today's high and low":{main:[[{node:`Merge`,type:`main`,index:2}]]},Limit:{main:[[{node:`Get weather`,type:`main`,index:0}]]},"1. Click 'Execute workflow’":{main:[[{node:`Set current day and month`,type:`main`,index:0},{node:`Set your location`,type:`main`,index:0},{node:`Set exchange currency`,type:`main`,index:0}]]},"Set current day and month":{main:[[{node:`Get a historical fact`,type:`main`,index:0}]]},"Set exchange currency":{main:[[{node:`Get exchange rates`,type:`main`,index:0}]]},"Get a historical fact":{main:[[{node:`Merge`,type:`main`,index:0}]]},"Get exchange rates":{main:[[{node:`Merge`,type:`main`,index:1}]]},"Select output for the email":{main:[[]]}}},h=`N8N_READY_TO_RUN_WORKFLOWS_DISMISSED`,g=i(a.EXPERIMENT_READY_TO_RUN_WORKFLOWS,()=>{let i=c(),a=t(),g=u(),_=n(),v=s(),y=l(),b=e(()=>[r.variantReadyToRun,r.variantReadyToRun2,r.variantReadyToRun3].includes(v.getVariant(r.name)?.toString()??``)&&y.userIsTrialing),x=o(h,!1),S=e(()=>x.value),C=()=>{x.value=!0},w=e=>{i.track(`User created ready to run workflows`,{source:e})},T=()=>{i.track(`User dismissed ready to run workflows callout`)},E=e=>{i.track(`User opened ready to run workflow`,{template:e})},D=(e,t)=>{i.track(`User executed ready to run workflow`,{template:e,status:t})};function O(){switch(v.getVariant(r.name)){case r.variantReadyToRun:return a.baseText(`workflows.readyToRunWorkflows.card`);case r.variantReadyToRun2:return a.baseText(`workflows.readyToRunWorkflows.card2`);case r.variantReadyToRun3:return a.baseText(`workflows.readyToRunWorkflows.card3`);default:return``}}function k(){switch(v.getVariant(r.name)){case r.variantReadyToRun:return a.baseText(`workflows.readyToRunWorkflows.callout`);case r.variantReadyToRun2:return a.baseText(`workflows.readyToRunWorkflows.callout2`);case r.variantReadyToRun3:return a.baseText(`workflows.readyToRunWorkflows.callout3`);default:return``}}return{isFeatureEnabled:b,isCalloutDismissed:S,createWorkflows:async(e,t)=>{let n=await g.createFolder(a.baseText(`workflows.readyToRunWorkflows.folder.name`),e,t),r={...d,parentFolderId:n.id},i={...f,parentFolderId:n.id},o={...p,parentFolderId:n.id},s={...m,parentFolderId:n.id};return await _.createNewWorkflow(s),await _.createNewWorkflow(o),await _.createNewWorkflow(i),await _.createNewWorkflow(r),C(),n},dismissCallout:C,trackCreateWorkflows:w,trackDismissCallout:T,trackOpenWorkflow:E,trackExecuteWorkflow:D,getCardText:O,getCalloutText:k}});export{g as t};
//# sourceMappingURL=readyToRunWorkflows.store-C3zGEzRw.js.map